...
def test_accuracy():
    ...
    # Test Case 7: Target at top-left
    ...
    # Test Case 8: Target at bottom-right
...
def test_time_complexity():
    ...
    # Verify O(m + n) complexity by checking if time growth is linear
    if len(times) >= 2:
...
def test_edge_cases():
    ...
    for i in range(300):
        ...
        for j in range(300):
            ...
            val += 1
        matrix.append(row)
    target = random.randint(0, 89999)
    result = solution.searchMatrix(matrix, target)
    print(f"Maximum size (300x300): {result} ✅")

    # Edge Case 2: Maximum constraint values
    matrix = [[10**9, 10**9 + 1], [10**9 + 2, 10**9 + 3]]
    target = 10**9
    result = solution.searchMatrix(matrix, target)
    assert result == True
    ...
    matrix = [[5] * 100 for _ in range(100)]
    target = 5
    result = solution.searchMatrix(matrix, target)
    assert result == True
...
def benchmark_solution():
    ...
    target = random.randint(0, 39999)

    start_time = time.time()
    result = solution.searchMatrix(matrix, target)
    time1 = time.time() - start_time

    print(f"Large dataset (200x200 matrix):")
    print(f"Time: {time1:.6f}s, Result: {result}")


if __name__ == "__main__":
    print("🧪 Testing Search a 2D Matrix II Problem")
    print("=" * 50)
    ...
    if complexity_passed:
        ...
        print("- Use search from top-right or bottom-left")
    else:
        print("\n❌ Tests failed - improve your solution's time complexity!")
        print("- Your solution appears to be worse than O(m + n)")
        print("- Consider using search from top-right or bottom-left")
