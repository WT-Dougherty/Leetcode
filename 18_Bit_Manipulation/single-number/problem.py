...
def test_accuracy():
    ...
    assert result2 == 4, f"Failed for nums={nums2}, expected 4, got {result2}"
    ...
    assert result6 == 3, f"Failed for nums={nums6}, expected 3, got {result6}"
...
def test_time_complexity():
    ...
    for size in test_sizes:
        nums = [i for i in range(size)] + [i for i in range(size)] + [size]
        # Shuffle the array
        import random
...
def test_edge_cases():
    ...
    result = solution.singleNumber([-1, -2, -1])
    print(f"Negative numbers: {result} ✅")

    # Edge Case 3: Large array
    nums = [i for i in range(1000)] + [i for i in range(1000)] + [1000]
    result = solution.singleNumber(nums)
...
def benchmark_solution():
    ...
    # Large array
    nums = [i for i in range(100000)] + [i for i in range(100000)] + [100000]

    start_time = time.time()
    result = solution.singleNumber(nums)
    elapsed_time = time.time() - start_time

    print(f"Large array (200,001 elements):")
    print(f"Time: {elapsed_time:.6f}s, Result: {result}")


if __name__ == "__main__":
    print("🧪 Testing Single Number Problem")
    print("=" * 50)
    ...
    if complexity_passed:
        ...
        print("- Consider using XOR operation")
